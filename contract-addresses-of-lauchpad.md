...
CREATE2 factory responsible for deploying ZNS Launchpad tokens.
...
Immutable Uniswap v4 Hook used by ZNS Launchpad pools on Base.
...
Holds the LP position permanently after launch.
...
Stores and distributes creator, referrer and treasury fee balances.
...
Applies dynamic fee protection during the initial launch period.
...
Stores time-locked creator token allocations.
...
Optional developer purchase extension used during token deployment.
...
Receives ZNS Launchpad protocol fees.
...
CREATE2 factory responsible for deploying ZNS Launchpad tokens on Robinhood Chain.
...
Immutable Uniswap v4 Hook used by ZNS Launchpad pools on Robinhood Chain.
...
Robinhood Chain uses a compatible UniversalRouter implementation with an additional `minHopPriceX36` field in `IV4Router.ExactInputSingleParams`.

Pass `0` to disable the additional minimum-hop-price check.
...
* Every supported network has an independent ZNS Launchpad contract deployment.
* All pools use WETH as the quote asset.
* No bonding-curve contract is used.
* No proprietary router is used on Base.
* Tokens launch directly into Uniswap v4 liquidity pools.
...
* Base uses the canonical Uniswap v4 UniversalRouter parameter structure.
* Robinhood Chain uses a compatible router with an additional `minHopPriceX36` parameter.
* LitVM LiteForge is a testnet deployment and is not included on this production-address page.
