---
description: ZNS Token  and Revenue Stream
cover: ../.gitbook/assets/Tokenomics.png
coverY: 0
---

# Tokenomics

This page outlines the official terms of the $ZNS Token Sale, including supply, pricing, distribution, vesting, and utility.

All data is aligned with what has been publicly shared with partners, media, and community channels.

***

### 🔹 Token Overview

• Token Name: ZNS Token

• Ticker: $ZNS

• Total Supply: 200,000,000 ZNS&#x20;

• Standard: ERC-20

• Network: Ethereum and BSC

• Smart Contract: Will be published publicly prior to TGE

***

### 🔹 Public Pre-Sale (5% Allocation)

• Total Tokens Available: 10,000,000 ZNS (5%)

Price Stages:

* Stage 1: $0.06 → Target Raise $100,000
* Stage 2: $0.07 → Target Raise $200,000
* Stage 3: $0.08 → Target Raise up to $400,000

Unlocking Structure:

* 50% unlocked at TGE
* 50%  vesting over 6 months

***

### 🔹 Market Metrics

• Initial Market Cap (IMC): \~$596,000

• Fully Diluted Valuation (FDV): \~$19,700,000

• Circulating at TGE: \~3.7% of total supply

***

### 🔹 Official Token Distribution

| Category                | % of Supply | Vesting / Notes                                      |
| ----------------------- | ----------- | ---------------------------------------------------- |
| Ecosystem               | 20.5%       | Long-term ecosystem expansion & rewards              |
| Marketing & CEX Listing | 15%         | Listings, campaigns, influencers                     |
| Foundation              | 10%         | 9-month cliff → 36-month vesting                     |
| Team                    | 10%         | 6-month cliff → 36-month vesting (standard and safe) |
| Partners & Advisors     | 10%         | 6-month cliff → 36-month vesting                     |
| Community Rewards       | 24%         | Raffles, quests, Layer3 campaigns                    |
| Liquidity               | 4%          | DEX + CEX liquidity provisioning                     |
| IDO Platforms           | 1.5%        | Launchpads, partners, allocations                    |
| Public Pre-Sale         | 5%          | 50% TGE + 6 months vesting                           |

***

#### **🔹 Token Utility**

The $ZNS token powers the entire multi-chain ZNS Connect ecosystem deployed across 80+ networks.

#### Utility for All Users

* Domain registration & renewals
* Domain marketplace trading
* Profile, identity & naming system
* API & SDK (developers & dApps)
* Governance voting
* Staking and ecosystem rewards

#### Exclusive Utility for Pre-Sale Investors

* 50% lifetime discount on all domain registrations
* Free GM & Deploy tools
* Free Token Launch tools
* Free NFT Launch tools
* Early Supporter Badge & perks inside the app

***

### 🔹 Official Resources

🔒 Smart Contract Audit —[ SolidProof](https://app.solidproof.io/projects/zns-connect)

📊 Analytics, Revenue & On-chain Stats — [Dune Dashboard](https://dune.com/zns/zns-connect)
